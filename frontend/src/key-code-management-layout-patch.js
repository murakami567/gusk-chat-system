const KC_API_BASE = "https://gusk-chat-system.onrender.com";
...
async function kcLoad() {
  const [pr, kr] = await Promise.all([
    kcFetch(`${KC_API_BASE}/properties`),
    kcFetch(`${KC_API_BASE}/admin/key-codes`, { headers: kcHeaders() })
  ]);
...
async function kcSave() {
  ...
  const res = await kcFetch(`${KC_API_BASE}/admin/key-codes/batch`, { method: "POST", headers: kcHeaders(), body: JSON.stringify({ property_name: kcState.property, room_number: room, items }) });
...
function kcRender() {
  ...
  root.innerHTML = `<div class="grid grid-cols-12 gap-4"><section class="col-span-12 lg:col-span-4 bg-white rounded-2xl shadow-sm border overflow-hidden"><div class="p-5 border-b"><h2 class="font-bold text-lg">物件一覧</h2><p class="text-xs text-slate-500">物件を選択</p></div><div class="p-3 space-y-2 max-h-[70vh] overflow-y-auto">${propsHtml || `<div class="p-6 text-sm text-slate-400 text-center">物件管理に物件がありません</div>`}</div></section><section class="col-span-12 lg:col-span-8 bg-white rounded-2xl shadow-sm border overflow-hidden"><div class="p-5 border-b flex justify-between gap-4"><div><h2 class="font-bold text-lg">${kcEsc(prop?.name || kcState.property || "部屋一覧")}</h2>${prop?.beds24_property_name && prop.beds24_property_name !== prop.name ? `<p class="text-xs text-slate-500 mt-1">Beds24表示名：${kcEsc(prop.beds24_property_name)}</p>` : ""}</div><button data-kc-add-room class="rounded-xl bg-blue-600 text-white px-4 py-2.5 text-sm font-bold">＋ 部屋を追加</button></div><div class="p-4 space-y-3 max-h-[70vh] overflow-y-auto">${roomsHtml}</div></section></div>${kcEditor()}`;
